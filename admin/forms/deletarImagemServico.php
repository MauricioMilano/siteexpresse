...
if(isset($_POST["submit"]) && isset($_POST["imagem"])){
	...
	else
	{
	    ...
	    if($stat=='ativo')
	    {
	    	foreach ($db->query("SELECT id_img, nome, status FROM imagens WHERE imagens.fk_pag = 3 ORDER BY id_img ASC") as $row)
    		{
    			$db->query("UPDATE imagens SET status = '' WHERE imagens.fk_pag = 3");
	    	}
	    	$aux = $row['id_img'];
...
?>
    ...
    <div class="col-lg-6 col-md-6 col-sm-8">
	    ...
	    <div class="row">
	    	<?php 
			foreach ($db->query("SELECT nome, status FROM imagens WHERE imagens.fk_pag = 3 ORDER BY id_img DESC") as $row){ ?>
			...
			<form method="POST" action="deletarImagemServico.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="nome">Imagem:</label>
			            <select name="imagem">
				            <?php 
			                    foreach ($db->query("SELECT id_img, nome FROM imagens WHERE imagens.fk_pag = 3 ORDER BY id_img DESC") as $row){ ?>
