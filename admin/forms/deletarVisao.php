...
$db = conecta_bd(); 
if(isset($_POST["submit"]) && isset($_POST["visao"])){
	$id = $_POST["visao"];
	    ...
	    <div class="row">
	    	<div class="table-responsive">
		    	<table class="table table-bordered">
		            ...
		            <tbody>
		                <?php 
							foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 8") as $row){ ?>
							<tr>
								<td>
			...
			<form method="POST" action="deletarVisao.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="visao">selecione para apagar:</label>
			            <select name="visao">
				            <?php 
			                    foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 8") as $row){ ?>
