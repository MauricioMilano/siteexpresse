...
$db = conecta_bd(); 
if(isset($_POST["submit"]) && isset($_POST["categoria"]) && isset($_POST["servico"])){
	$id = $_POST["categoria"];
	$servico = $_POST["servico"];
	$do = $db->query("DELETE FROM textos WHERE textos.id_texto = '$id' AND textos.fk_topico = '$servico'");	
	if ($do) {
...
?>
    ...
    <div class="col-lg-6 col-md-6 col-sm-8">
    	<p><strong>Selecione a categoria que deseja remover</strong></p>
	    <div class="row">
	    	<div class="table-responsive">
		    	<table class="table table-bordered">
		            ...
		            <tbody>
		                <?php 
							foreach ($db->query("SELECT titulo, id_texto FROM textos WHERE textos.fk_tipo = 13 AND textos.fk_topico = '$servico'") as $row){ ?>
							<tr>
								<td>
			...
			<form method="POST" action="deletarCategoriaServico.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="categoria">selecione para apagar:</label>
			            <select name="categoria">
				            <?php 
			                    foreach ($db->query("SELECT titulo, id_texto FROM textos WHERE textos.fk_tipo = 13") as $row){ ?>
