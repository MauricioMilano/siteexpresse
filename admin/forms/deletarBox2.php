...
$db = conecta_bd(); 
if(isset($_POST["submit"]) && isset($_POST["box"])){
	$id = $_POST["box"];
	    ...
	    <div class="row">
	    	<div class="table-responsive">
		    	<table class="table table-bordered">
		            ...
		            <tbody>
		                <?php 
							foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 3") as $row){ ?>
							<tr>
								<td>
			...
			<form method="POST" action="deletarBox2.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="box">selecione para apagar:</label>
			            <select name="box">
				            <?php 
			                    foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 3") as $row){ ?>
