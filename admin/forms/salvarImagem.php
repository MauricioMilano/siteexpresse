    ...
    <div class="adm-alert col-lg-12 col-md-12">
        <?php           
            ...
            if ((isset($_POST["submit"])) && (! empty($_FILES['foto']))){
                ...
                if(isset($nomeImg)){
                    $db = conecta_bd();
                    foreach ($db->query("SELECT id_img, nome, status FROM imagens WHERE imagens.fk_pag = 1 ORDER BY id_img ASC") as $row){
                        $db->exec("UPDATE imagens SET status = '' WHERE imagens.fk_pag = 1");
                       
                    }
                    $do = $db->exec("INSERT INTO imagens (fk_pag, nome, status) VALUES (1, '$nomeImg', 'ativo')");
                    if ($do) {
