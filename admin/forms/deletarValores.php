...
$db = conecta_bd(); 
if(isset($_POST["submit"]) && isset($_POST["valores"])){
	$id = $_POST["valores"];
	    ...
	    <div class="row">
	    	<div class="table-responsive">
		    	<table class="table table-bordered">
		            ...
		            <tbody>
		                <?php 
							foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 9") as $row){ ?>
							<tr>
								<td>
			...
			<form method="POST" action="deletarValores.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="valores">selecione para apagar:</label>
			            <select name="valores">
				            <?php 
			                    foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 9") as $row){ ?>
