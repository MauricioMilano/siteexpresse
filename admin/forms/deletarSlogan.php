...
$db = conecta_bd(); 
if(isset($_POST["submit"]) && isset($_POST["slogan"])){
	$id = $_POST["slogan"];
	    ...
	    <div class="row">
	    	<div class="table-responsive">
		    	<table class="table table-bordered">
		            ...
		            <tbody>
		                <?php 
							foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 1") as $row){ ?>
							<tr>
								<td>
			...
			<form method="POST" action="deletarSlogan.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="slogan">selecione para apagar:</label>
			            <select name="slogan">
				            <?php 
			                    foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 1") as $row){ ?>
