...
if(isset($_POST["submit"]) && isset($_POST["imagem"])){
	...
	if (!unlink($arquivo))
	{
	  echo '<div class="alert alert-danger" role="alert">Ocorreu um erro ao tentar apagar. Contate o desenvolvedor do sistema</div>';
	}
	else
	{
	    if($stat=='ativo')
	    {
	    	$qry = $db->query("DELETE FROM imagens WHERE imagens.nome = '$aux'");
		    foreach ($db->query("SELECT id_img, nome, status FROM imagens WHERE imagens.fk_pag = 5 ORDER BY id_img ASC") as $row)
    		{
    			$db->query("UPDATE imagens SET status = '' WHERE imagens.fk_pag = 5");
	    	}
	    	$aux = $row['id_img'];
			$do = $db->query("UPDATE imagens SET status = 'ativo' WHERE imagens.id_img = '$aux'");
...
?>
    ...
    <div class="col-lg-6 col-md-6 col-sm-8">
	    ...
	    <div class="row">
	    	<?php 
			foreach ($db->query("SELECT nome, status FROM imagens WHERE imagens.fk_pag = 5 ORDER BY id_img DESC") as $row){ ?>
			...
			<form method="POST" action="deletarImagemParceiros.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="nome">Imagem:</label>
			            <select name="imagem">
				            <?php 
			                    foreach ($db->query("SELECT id_img, nome FROM imagens WHERE imagens.fk_pag = 5 ORDER BY id_img DESC") as $row){ ?>
