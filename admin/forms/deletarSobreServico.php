...
$db = conecta_bd(); 
if(isset($_POST["submit"]) && isset($_POST["sobre"])){
	$id = $_POST["sobre"];
	    ...
	    <div class="row">
	    	<div class="table-responsive">
		    	<table class="table table-bordered">
		            ...
		            <tbody>
		                <?php 
							foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 12") as $row){ ?>
							<tr>
								<td>
			...
			<form method="POST" action="deletarSobreServico.php" enctype="multipart/form-data">
			    <fieldset>
			        <label for="sobre">selecione para apagar:</label>
			            <select name="sobre">
				            <?php 
			                    foreach ($db->query("SELECT conteudo, id_texto FROM textos WHERE textos.fk_tipo = 12") as $row){ ?>
